...
//   satisfaction  patience, the bonus: honks cost it, nothing about it fails
//                 a level (Devon's brief: bonus scoring, not a third fail)
//
...
export function meters(world) {
  ...
  const honkPenalty = Math.min(0.6, s.honks * 0.03);
  ...
  return {
    ...
    collisions: s.collisions, honks: s.honks,
...
export function score(world) {
  ...
  const points = Math.round(m.cleared * 10 + (survived ? 200 : 0) + stars * 150 + bonus * 2 - m.collisions * 100 - m.honks * 5);
