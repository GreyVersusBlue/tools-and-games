...
**These are inert.** They appear on core content, they display on the sheet, and no code reads them. Six of them were listed as implemented in earlier versions of this guide, which is worse than not documenting them at all — an author reasonably assumed a feat carrying one would do something. `assurance` and `surprise-attack` were two of the six; both are fixed now (moved to the working list above).

| Hook | On | Status |
|---|---|---|
| `racket-scoundrel` | Rogue's Scoundrel racket | Flavour only — the other two rackets work. Needs a Feint action, which the combat engine doesn't have at all yet (no button, no `resolveTargeted` case); this is the actual gap, not the hook id itself. |
| `edge-outwit` | Ranger's Outwit edge | Flavour only — Flurry and Precision work. The AC half (+1 circumstance vs. your hunted prey) is a plausible next session's work: one more term in `effAC`. The skill half (+2 to Deception, Intimidation, Stealth against your prey) is a bigger gap — combat has a Demoralize action (Intimidation) that could take the bonus, but no Feint or Hide, so Deception and Stealth have nowhere to attach it yet. |
| `mobility` | Mobility feat | Checked this session and it's not just unwired, it's **currently unwireable**: `provokeAlong()` (the only reactive-strike trigger in the engine) has `if(mover.side!=="foe") return;` at the top — reactions only ever fire against a moving *foe*, never against the hero or a companion. No monster in the Registry carries `reactive-strike` either. Mobility exists to protect a mover from a reaction that, as shipped, nothing in this game can ever make against the party. Implementing it today would be a flag nothing reads; the real prerequisite is giving at least one monster a reach reaction, which is a monster-data question, not an engine one. |
| `crossbow-ace` | Crossbow Ace feat | Flavour only. Needs a turn-scoped "reloaded this turn" flag on the ranged attack (the `reload-1` trait is on the item already, `crossbow.reload-1`, but nothing currently spends an action on reloading — Strike with a loaded crossbow just works) plus a damage-die bump and +2 circumstance conditioned on that flag or on `huntPreyId`. Real work, not a one-liner; the hunted-prey half alone would be inconsistent with the feat's own "or after reloading" wording. |
