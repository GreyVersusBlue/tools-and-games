...
// T6 — the whole harness, parameterized by which period's data/ folder to
// load. Period 1 also runs the three-chart T4 demonstration; a later period
// just needs to prove its own lesson and roster produce sane numbers.
function runPeriod(base, { showCharts = false } = {}) {
  const D = f => JSON.parse(fs.readFileSync(`${base}/${f}.json`, 'utf8'));
  ...
  console.log(`lesson: ${lData.beats.reduce((a, b) => a + b.seconds, 0)}s of authored beats ` +
    `(+${lData.filler.seconds}s filler) against a ${CFG.periodSeconds}s period\n`);
...
runPeriod('../data', { showCharts: true });

console.log('=== PERIOD 2 ===');
runPeriod('../data/period2');
