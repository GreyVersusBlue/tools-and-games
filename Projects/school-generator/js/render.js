...
export function initRender(canvas) {
  ...
  // Which camera is driving, and whether a photograph is being composed.
  //
  // Both are declared up here rather than beside the composer they mostly
  // serve, because the glazing below reads them while it is being built — and
  // a `let` read before its declaration is a TDZ error on the one path nobody
  // automates, which is a mistake this codebase has already made once (see
  // Phase 30's `galleryFilled`).
  let mode = 'edit';
  ...
  // the sky's own palette asked for, the depth-of-field pass, and (since
  // Phase 31) whether the glass refracts. Everything here is view state — none
  // of it is saved with the design, because a photograph is not part of the
  // building.
  const photo = {
    on: false,
    fov: 60,
  ...
  //
  // **What it costs, measured.** A transmissive material makes three.js render
  // the whole scene a second time into a target the pane samples, and on a
  // fill-bound renderer that is not a rounding error: timed over a corridor on
  // the software rasterizer CI uses, it took a frame from 547ms to 1,186ms —
  // **it more than doubles the cost of a walk**. (The relief maps, on the same
  // view, cost 13%.) So it lives exactly where depth of field lives, for
  // exactly the reason stated there: it is a second scene render, so it is
  // paid for only while a photograph is actually being composed. Ordinary
  // walking keeps the blended pane it has always had — and keeps the one thing
  // that has to survive without a transmission pass, which is that you can see
  // through a window and not through a frosted one; that is `blend` in the
  // glazing table. See `applyGlassMode`.
  //
  // `side` is FrontSide rather than DoubleSide, and that is not a detail: a
  // pane is a closed box, so the back faces were never visible, and a
  // DoubleSide transmissive material makes the renderer flip `needsUpdate` on
  // it twice a frame to draw its own backside.
  ...
  // A pane refracts while a photograph is being composed, and blends the rest
  // of the time. Two reasons, and the second is the one that decided it: at
  // the drafting table the camera is 200ft up and a window is a line, so a
  // second scene render would buy nothing at all; and in an ordinary walk it
  // would buy refraction at more than double the frame cost, on a tool that
  // runs on phones and in a headset. Photo mode is where somebody is looking
  // at the glass rather than walking past it, and it is already paying for a
  // second render for the depth of field.
  function applyGlassMode(m) {
    const g = m.userData.glazing;
    if (!g || m.userData.flat) return;
    const refract = mode !== 'edit' && photo.on;
    const want = refract ? g.transmission : 0;
    const opacity = refract ? 1 : g.blend;
    if (m.transmission === want && m.opacity === opacity) return;
    m.transmission = want;
    m.opacity = opacity;
    // Crossing zero adds or drops USE_TRANSMISSION, which is a recompile — so
    // it happens when the shutter opens and never per frame.
    m.needsUpdate = true;
  }
  ...
  function setMode(m) {
    ...
    // Phase 31: leaving a walk turns photo mode off, so the glass has to be
    // told the shutter closed.
    applyGlassToAll();
  ...
  return {
    ...
    setPhoto(patch) {
      ...
      // Phase 31: the glass refracts for the photograph and not for the walk.
      // Same second scene render, same bargain as the depth of field above.
      if (photo.on !== wasOn) applyGlassToAll();
      applyPhotoCamera();
      return { ...photo };
    },
