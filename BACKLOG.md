...
**The last batch of ranked work that shipped** is **Orbital's level generator
(PR #299)**, the old rank 1 alone — a 1 in one area whose only ¼ row wants a
real device, which is a batch on its own under the size table. The row named
Claude Fable 5.1 and was worked under Fable 5.1. That is the line to update
when your batch merges; a PR that only changes these files is not a batch and
does not belong in it. **33 ranked items remain**, and **every one of them
names a model.**
...
**Orbital rolls its own sectors** (#401 to #405). The sector map has a
"Random sector" row, Easy, Medium and Hard, and a roll is 0.8 to 1.7 s under
software Chromium. `js/generator.js` proposes from a tier and a seed; it does
not decide. `OrbitalCode.validate` says whether a candidate is a level, and a
census of 1,200 launches flown through `solve` says whether it is worth
playing. **The census grid is every second angle and every second power of the
CI search's grid**, so a census win is one `findWinningShot` reaches by
construction, not a second verdict (#399 holds). **What the win fraction
measures is how forgiving a level is, not how hard**: in an empty field it is
the marker's angular size, and the 22 shipped levels run 0.75% to 12.7% with
no relation to pack order. What tells decoration from a level is the second
number, how many winning launches would also win with every body removed:
15 of 22 shipped levels score 0, First Light scores 18 of 18, and The Long
Way 9 of 9 (every winning shot skips both portals). A rolled sector is a
shared one: no `key`, no stars, Remix works, and the address bar carries it
under `#l=` the moment it exists. `test/generator.mjs` is the third command
in Orbital's CI entry, 33 s, and it reads the judge's words against all 22
shipped levels as a fixture, so a band cannot move without a shipped level
changing sides in that table.
...
**Rank 1 is `Projects/Closing Time` — multi-offer escalation wars as a
dedicated flow.** A **1** on **Claude Opus 5**. A 1 may take up to two ¼ rows
from the same area with it, and Closing Time has none (its other rows, 2 and
14, are both 1s), so it goes alone. Orbital has had three batches in a row and
has nothing ranked above 31 now.
The next half is rank 8. Eleven of the thirty-three are ¼ — ranks 9, 11, 16, 17,
18, 21, 22, 23, 24, 29 and 32. **Five of those eleven want hardware nothing
here has** (16, 18, 23, 24, 32), as do three of the halves (8, 15, 25); the
Parked section below the table says why they were left ranked anyway.

**The model split is 11 Opus 5, 13 Fable 5.1, 9 Sonnet 5.** The rubric is in
...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | Multi-offer escalation wars as a dedicated flow | `Projects/Closing Time` | 1 | Opus 5 | `claude/stoic-allen-tt4xsg` | [Closing Time](#closing-time) |
