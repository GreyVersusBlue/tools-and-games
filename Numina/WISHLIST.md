...
- [x] **A packet builder page**, `/mechanics/packet/`. 49 chapters offered,
  generated from `nav.json` and the nations collection so the list cannot
  drift; `src/js/packet.js` fetches the published HTML and assembles a cover, a
  combined contents and every chapter on its own sheet. Each chapter's `h1`
  becomes an `h2` and everything under it shifts a level, every id gains the
  chapter's slug, and a link to another chapter in the same packet is rewritten
  to a fragment.
- [x] **Three prebuilt packets** in `src/_data/packets.json`. The New Player
  Kit and the NPC packet are `?p=` links that tick the boxes and assemble on
  load. The Combat Card is a link to `/mechanics/combat-reference/`, which
  already prints as a card sheet and is unchanged — `smoke.mjs` fails if that
  page ever stops being `cardsheet: true` (#321).
- [x] **A service worker**, generated by `tools/service-worker.mjs`: 81 files,
  every page, both stylesheets, all eight scripts, the five woff2, the two
  assets and Pagefind's fixed-name files. The version is a content hash that
  excludes `pagefind/`, whose chunk names are not stable across machines
  (#322); the language metadata chunk is read out of `pagefind-entry.json` at
  install time and the index chunks are runtime-cached.
- [x] **Say when it is stale.** `src/js/offline.js` on every page: an offline
  banner naming the build on the device, and an "Update now" button when a
  newer one is waiting. The worker never calls `skipWaiting` on install. The
  packet page carries the same controls as a section.
- [x] **`clean.mjs` and `smoke.mjs` moved together.** `sw.js` is in both
  `GENERATED` lists, which puts it in the hygiene check. The manifest is pinned
  against the built pages, against `main.css`'s own font references, and
  byte-for-byte against what the generator writes now.

**Continuous page numbering is a `@page packet` margin box** (#323), settled by
rendering PDFs rather than reading a support table: Chromium honours it,
Firefox falls back to its own print footer, and `target-counter()` — the one
thing that could put a page number beside a contents entry — is in no shipping
browser, so the contents lists chapters only. It is a named page so the six
chapters that already print one at a time are untouched.

**`test/a11y/packet.mjs` is new**, 19 assertions in the pinned Chromium: the
assembly, the page counter surviving in the CSSOM, and the site opening with
the origin shut down. Shutting it down is the point — two guard-rails passed
