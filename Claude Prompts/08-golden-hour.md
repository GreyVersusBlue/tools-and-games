...
| `Tools/board-check/**` | Shared dev tooling, including `play-games.mjs` (drives this game), `games.mjs` (enters it) and `harness.mjs`/`drive.mjs`. Prompt 21. |
...
4. **Not your bug, but know about it: the wading/footprint beats have now been re-verified on a
   ...
   which is prompt 21's file. Flag it in your shared-file requests if it isn't already fixed by the
