...
Snapshot of `Claude Prompts/` immediately before prompt 23's round-3 refresh, taken 2026-08-03.
Site version at the time: **11** (`index.html`'s footer read "version 11 · the compass points
true"). Current handoff at the time: `gvb-site-handoff-v10.md`.

This is the round-2-refreshed set of twenty-two prompts, plus every notes file the round-3
sessions wrote (all twenty-two, including prompt 22's own full pass — no partial-pass gate this
round).

## Which prompts produced a notes file

All twenty-two did, including prompt 22 (General Site Improvements) and prompt 23's own
predecessor notes file. Prompts 01 and 15 did not run this round — they were already in
`Stable/` and correctly skipped, per the cycle's own convention. Their notes files in this
archive are unchanged copies of what round 2 left.

## What shipped, one line per project

| # | Project | What shipped |
| --- | --- | --- |
| 01 | Anathema Archive (Stable) | Did not run — skipped per `Stable/` convention. Re-verified this refresh: still nothing outstanding, `Pathfinder/data/` still an open question. |
| 02 | Pathfinder Campaigns | Zero edits, zero findings — first fully clean verification round. Whether that alone is enough for `Stable/` (01's bar) or whether a second clean round is needed (15's bar) is now an open question for Devon. |
| 03 | Pathfinder Characters | Same as 02 — first fully clean round, same open Stable-timing question. |
| 04 | Aphelion | Zero code changes; confirmed the `#signal` regression beat is still blocked on an airlock-entry beat that doesn't exist yet. |
| 05 | Castle Conundrum | Fixed four objects sealed in the back wall (table, statue, and two more found this round — a cabinet and a commode), reviewed and promoted the preview candidate, fixed `play-castle.mjs`'s own engine-mismatch bug, fixed the gate door's hinge/pivot math. No known open bugs as of this round. |
| 06 | Closing Time | Threaded an exact `recId` through the Ledger filter (was a display-name substring match) and answered the career-ending "what's next" question. |
| 07 | The Fourth Quarter | Built the spoilage mechanic, answering this project's own "Questions for Devon" difficulty-curve question directly (rent-by-tier was the cheap half from round 2; spoilage is the rest). |
| 08 | Golden Hour | Rebuilt dune grass as camera-facing quads, closing the last obviously-flat-looking thing on the project. |
| 09 | Faire Weekend | Closed the mobile-tap-target gap (48px cells, `#board`'s desktop column split) and found one more real wiring gap (`cancelMove`, untested until this round) even after round 2 called the wiring audit closed. |
| 10 | Torchbearer | Wired edge-outwit, a real Feint action, and a reload mechanic — closing all three feature items round 2 left open. Did the `mountSaveBar` cleanup. Only `mobility` (blocked on monster data) remains. |
| 11 | The Absalom Inheritance | Built a real character-creation pick screen with a second build (Fighter, alongside the original Wizard) — closing what round 2 called the single highest-value gap. |
| 12 | Coffee Shop Sim (Corner & Kettle) | Fixed its own `test/drive-save.mjs`'s `waitForFunction` bug (9 instances, one beyond what a grep first found) and measured a real design question: whether the Serve button should require full completion now that baristas are the main path to a finished cup. |
| 13 | Daredevil | Closed all four of round 2's remaining tasks (gzip measurement, absent-relationship sweep, touch-control verification, contrast fix) and found the single biggest open item on the site: "Not interested" to Earl leaves the entire milestone spine almost unchanged despite removing three optional cards. |
| 14 | Integer Foundry | Fixed its own `test/browser.mjs`'s engine-mismatch bug, played the difficulty curve for feel, and made the case the two remaining model gaps are one coupled piece of work, not two. **Moved to `Stable/`** — nothing outstanding. |
| 15 | The Fracture Cycle (Stable) | Did not run — skipped per `Stable/` convention. Re-verified this refresh: still nothing outstanding. |
| 16 | Final Grade Checker | Asked Devon directly and found the grading bug was bigger than round 2 thought: correct quality-point thresholds are whole numbers (4/3/2/1/0), not `.5`-offset midpoints — every `x.75` average had been one full letter too high since the tool's dual-method calculation first went live. |
| 17 | Image → PDF Assembler | Re-verified the rotation fix fresh against a new real PDF; made no code changes. **Moved to `Stable/`** — nothing outstanding beyond two environment-blocked verification checks (a real phone photo, a compositing browser pane). |
| 18 | Name Picker | Fixed `test/browser.mjs`'s own engine-mismatch bug; found the rename question now targets `newindex.html`, not `index.html`, since the board no longer links to this file directly. |
| 19 | Schedule Visualizer/Browser | Did the restructure round 2 unblocked: 863,737 bytes in one file is now a 124,566-byte shell plus a seven-file `Tools/schedule/app/` split, verified byte-for-byte against the original. Read the What-If Lab and both floor-plan renderers for the first time; both are genuinely non-duplicative, not restructure candidates. |
| 20 | Seating Chart Generator | Fixed rotated desk labels (now counter-rotate) and all three Playwright-only bugs in its own test file (one beyond what was flagged). First refresh with nothing carried over from this project's own work. |
| 21 | Orbital | First real round: a physics test suite (all 22 levels confirmed winnable, wormhole/booster/solidity/save-migration checks), a reset-confirmation fix, and a preview/OG card. Found the live level count was 22, not the 21 a previous refresh recorded. |
| 22 | General Site Improvements | Single full pass (all 21 notes existed first). Root-caused and fixed the repo-wide `sync-social-tags.mjs` false-DRIFT (Windows `core.autocrlf` line-ending mismatch, reported by 16 of 21 projects), fixed two real bugs in shared test tooling from a fair environment (Golden Hour's wading beat, The Fourth Quarter's Real Estate walk), extended `check-integrity.mjs`'s offsite sweep to `.js`/`.css`, gave Orbital its preview/OG. Version bumped 10→11, wrote `gvb-site-handoff-v10.md`. |
| 23 | Refresh prompts | This file's own prompt. Round 2's refresh is archived at `archive/round-2/`. |
