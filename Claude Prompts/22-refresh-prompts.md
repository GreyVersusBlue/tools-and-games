...
- **A project whose notes say it is finished** — some prompts explicitly invite that answer, and for a
  ...
  N**, with the date and what was checked. Do not delete the prompt. Keep it so a future round can
  pick it up, and list it in your notes as droppable from the next round so Devon can skip it.
...
- Any boundary table row that changed.
- Any note about counts that moved — the round-one README flags that deleting the Bestiary Gallery
  ...
  than accumulating.
- A short "rounds so far" section: what round N covered, which projects are done, which are next.
  One line each.
...
- Confirm the archive is a complete copy, including `notes/`, and that nothing in the live folder was
...
- **Projects that are done** — droppable from the next round, with what you verified.
