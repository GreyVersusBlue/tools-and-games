...
generated PDFs. This prompt is self-contained.
...
1. This whole file.
2. **`Claude Prompts/notes/17-image-to-pdf-notes.md`** — round 2's session: the rotation control,
   verified against real generated PDFs by extracting and decoding the raw JPEG byte streams, not
   just reasoning about the canvas math. Round 1's notes are archived at
   `Claude Prompts/archive/round-1/notes/17-image-to-pdf-notes.md` — jsPDF vendoring, JPEG/WEBP
   input, quality presets, reorder/remove, the EXIF-orientation fix.
...
Round 2 closed the scanner-rotation gap. What's left is the same two items round 1 left,
unchanged:

1. **Verify the EXIF fix against a real sideways phone photo.** Two rounds running without a camera
   in the sandbox. The fix is reasoned correctly from documented browser auto-rotation behavior but
   has never been checked against an actual photo. Five minutes at a real device would close this
   permanently — reasoning from spec is not the same standard of proof this tool otherwise holds
   itself to.
2. **A real screenshot of the two-row mobile layout.** Geometry-verified (button sizes, no
   horizontal overflow) but not eyeballed, across two rounds, because the sandbox's browser pane
   hasn't been compositing frames for a screenshot in either session.
3. **Settings persistence, only if an actual teacher asks for it.** Still just a convenience, still
   ...
   `gvb-save.js`.
4. **If genuinely nothing else surfaces:** this tool now covers both of the original audit's
   rotation problems on top of everything round 1 already closed. The remaining open items are
   verifying an assumption against a real device and getting an actual look at the layout — not
...
- `cd Tools/board-check && npm run check` → as of this refresh: **335 units, 0 broken; 0 collisions
  across nine widths, tightest vertical gap 3.5px.**
- `npm run social:check` → **17 notices, 17 already current** (dropped from 22 this round — a real,
  correct count, not a regression, and the parse failure that blocked this check in round 2 is now
  fixed too).
