...
**#635. Above the fog line a Blue Hour beat changes kind by passing from the woods to the mountain: the silence stills the wind, and the snap is a stone.** Past `STILL_AIR` (0.6 of `summitAir`, the band main.js weathers by) `audio.js` already played no birdsong, so the silence beat up there was spending its 22 to 32 s on birds that were not there; the only thing left of it was the music duck and a crow. Now it takes the wind instead, bed and canopy, and lets go with one hard gust in place of the crow. A branch breaking becomes a stone let go upslope, five to seven knocks panned from the uphill ear to the valley one on `fallLine`, each lower, duller and quieter. Both are decided where they are staged, from one threshold in `field.js` that the birds read too. Inside dread.js's doctrine and Blue Hour item 1's line: no save, no verbs, nothing collected, nothing that answers being investigated. The shape and the eyes were left for the next increment because a visual beat in summit fog wants pixel checks this container can only draft (#630). Reversible by pointing `summitKind` at `() => false`. *Blue Hour, rank 8.*

**#636. Signal City's reference hand weighs a queue by its waiting, leaves a timed plan alone, and holds a green no longer than 25 s.** R1's hand (`tools/calibrate.mjs handStep`) picks, at each green's end, the phase whose stopped cars add up to the most of one-plus-seconds-waited. By count alone a lone W-L trucker waited 180 s and locked Four Ways on 4 of 6 seeds. It holds the running green for its own queue to 25 s (`HAND_MAX`), under the 30 s `boxStall`: at 45 s a permissive left waiting mid-box locked First Light seed 5. It does not press phases on a timed plan, because on Two Blocks the plan is the lesson and the hand's part there is the offset, swept every 4 s at load through `World.setOffset`. A gap-out (end a green once its standing queue is gone) was tried on the ruled levels and dropped: with the corridor off it took Rush Hour's greedy from 71.8 cleared at 11.8 s to 65.8 at 13.4 s. The hand counts what a player sees, every stopped car, rather than `World.queued`, which answers for a loop. Reversible by editing `greedy`; nothing but the tool reads it. *Signal City, R1.*
...
**#639. Signal City's second star is the lesson on a level with a default, not worse defaults, and Two Blocks ships on one clock.** `lesson: { kind, ... }` replaces the wait target on Rush Hour (the ambulance on time on its corridor), Main Street (no split platoon), Two Blocks (at most 50% of handed-on cars stopping again) and School Run (no walk call over 40 s), because no input met the old wait targets while the lesson's move cost them. Two Blocks ships at offset 0 because at its old 16 s no input already made the progression (at 0, 65 to 95% of handed-on cars stop again; at 16, 19 to 71%), and Main Street's `boxStall` went from 45 to 60 because a held procession kept a permissive left in the box 48 s. School Run misses the hand half of the rule (0 of 6 three-starred, one lock); the long form, with every number, is `git show 166f584:HISTORY.md`. *Signal City, R2.*

**#640. A star earned under the old second star stands.** `recordResult` keeps the best, so a Rush Hour three-star from the wait target, and the shop stars spent from it, survive a one-star run under the lesson. Clawing them back would mean a save migration for stars that were earned by the rules of the day, and stars stay 0 to 3 either way, so `signal_city_v1`, the save's shape and `repair` do not change (#36, #37). *Signal City, R2.*

**#641. Rush Hour is Signal City's one hard level, not School Run.** R3's test was the reference hand clean on six seeds: School Run fails it (collisions on seeds 4 to 6, a lock on 3), and so do Main Street (seeds 4 and 5) and Two Blocks (seed 1, twice), while Rush Hour's hand three-stars 6 of 6 with none and no input is clean there too. Its one crash is the lesson played on the shipped 22 s rule: the corridor puts the W left's arrow into an E through still in the box on seeds 2, 5 and 6 at 1.5 s of all-red and on none at 2.5 s, so the all-red is the survival tool the row asked for. Reversible by setting `mode: 'soft'` back. *Signal City, R3.*

**#642. Blender-made assets are the top priority, and every Blender row ranks above every other row.** Devon's instruction of 2026-09-25, and the re-rank is his: CLAUDE.md keeps re-ranking the list wholesale off a session's list, and this is the one exception he authorized, not a change to that rule. Blender runs only on Devon's local Windows machine, headless (`blender -b -P script.py`), never in a cloud container, so a Blender row is not parked under rule 2: it stays ranked, and a session without `blender` on PATH skips it and takes the next row. The thirteen rows that were already ranked kept their relative order below the new ones. *The repo.*

**#643. Each project keeps its own copy of the Blender `common.py` and of its validator, and there is no shared copy.** #17 is about one project's change silently moving another, and that reaches a dev-only script whose output is the bytes a game ships: one fix to a shared `common.py` would re-export every other project's assets differently the next time any of them re-ran. #619 is not a counter-case: `asset-pipeline.mjs` is a post-process with one owner, reading originals pinned at a commit, while `common.py` is the authoring step, and the projects already disagree about what it must write (Bell to Bell's CLAUDE.md wants a `.gltf` that its meshopt recipe compresses, School Generator's `gltf.js` reads vertex colour and no texture, and a 2D game wants a sprite sheet). A copy's header names the project and commit it was copied from; the site's diorama copy lives in `Tools/board-check/blender/` as the site's own (#43, #51). *The repo.*

**#644. The ranked table has a `Gate` column: what the session's machine needs before it can take the row.** The values are `blender` (on PATH, locally), `GPU`, `phone` and `ears`; blank means a cloud container can do it. A column and not a Detail prefix, because Detail is a link read last and a gate has to be seen before the claim. The hardware rows got their values from their own text and nothing else about them changed; only a `blender` row is skipped in place (#642). *The repo.*

**#645. A Blender row commits its output, and everything downstream of it is cloud work.** The GLTFLoader rows, the wiring rows and every validator read committed files, so they carry no gate, a validator joins its project's CI, and a wiring row is taken only once the pack row above it has merged. What stays local is authoring: running the scripts, rendering the contact sheet, and showing that a second run changes nothing (`git status --porcelain` empty after a rerun). The contact sheet goes to a gitignored `tools/blender/out/` and is not committed, because every committed file is a live URL and the `.glb` or sheet itself is what a reviewer loads. *The repo.*

**#646. A Blender pack replaces what a game already shows and adds nothing it does not.** Blue Hour's bear stays the dread beat's silhouette plane and its elk stays a bugle, because not seeing either one clearly is the point. Hearth's faces stay procedural, because every face is pixel-identical to the one its seed drew before (`render.js`), and Fourth Quarter's walls and rails stay built from its layout, because the room is a description (Phase 1). Something a game does not show yet is a design row, not a pack row. *The repo.*
...
## Golden Hour's night moves and Blue Hour's fall line, 2026-09-24 (PR #395)
Ranks 5 and 9, two ½ rows in two areas, both naming Fable 5.1, worked under Opus 5.5. Ranks 1 to 4 and 6 to 8 need a real GPU, a phone or ears. Rank 5 was conditional on night proving popular; the condition sets priority, not what the page is, so it was built (#633). `js/creatures/nightpaths.js` holds both movers as pure clocks beside `field.js`, which now owns `CAMP` (campfire.js re-exports it). The fourteen fireflies nearest the camp, 25 to 41 m out, leave the hollows on their own onsets from nightT 0.2 and are all 3 to 6.5 m from the fire by 0.60, each bowing up to 4 m off the straight line. The owl hunts every 80 to 140 s of night: a glide that steepens into a stoop 12 to 20 m from its snag, on dry dune sand and on the side away from the walker, 1.2 s in the grass at 0.15 m, and a climb back to the nearer snag, never over 4.7 m/s. The first draft of the onsets (0.26 to 0.44, span 0.2) put all fourteen in transit at once; the "ones and twos" check caught it. Rank 9 went the other way from "accept the zero": `downhillAt` is `fallLine`, the raw hillside's 4 m gradient (#634). The pan reads ±0.835 at the suite's staged stop, toward the valley, and the descending shape now shows a clear profile in 7 of 8 stagings where session 6 measured 0.02 to 0.25. `smoke.mjs` 117 to 135 in Golden Hour, 109 to 113 in Blue Hour, `test/browser.mjs` 95 to 96 (96/0 under Xvfb, real-time beats included, #53 says that proves nothing either way), `play-games.mjs golden-hour` 30 to 33 with the 7 real-time failures `main` already had. Breaks from green, each caught by its own line: in Golden Hour's smoke, fireflies drawn farthest-first, `ringMin` 1.0, one onset for all, `span` 0.5 (the arrival line plus two), `bow` 40, drift height ignoring the dunes (-2.64 m), hunt targets off the dunes, the ground clamp removed (0.149 m, a millimetre under its floor, and the suite says so), `grass` 0.6, no time in the grass, `dur` 3 (the speed line, 15.5 m/s) and hunting beside the walker (3.7 m). In Blue Hour's smoke, the trail tangent (the sideways line, median 0.00), the uphill sign, the benched `groundHeight` (the up-mountain line, -0.37) and an unnormalised vector. In `browser.mjs`, the trail tangent back (`pan -0.000` both ways) and the fall line reversed (the valley line alone). In `play-games.mjs`, run together: ten flies drawn (10 near the fire, not 12), the owl out from nightT 0.6 (`owlHunt` answered with a target at sunT 1230), and `grass` 1.0 (1.00 m over the sand; the day break had already sent the owl off its snag, so the perched reading in that run was mid-hunt, and the line failed on the drop regardless). Commit before breaking; the breaks ran in `git worktree` copies so the branch was never dirty.

## Blue Hour, beats in kind above the fog line, increment 1, 2026-09-24 (PR #397)

Rank 8, a 1 naming Fable 5.1, worked under Opus 5.5; the row stays open. The call (#635): above the fog line a beat changes kind by passing from the woods to the mountain. The evidence for where to start was in `audio.js`: birdsong already stopped at altT 0.6, so the silence beat's whole point was inaudible on the last 19% of the trail. `field.js` owns the band now (`FOG_LINE`, `summitAir`, which main.js's local smoothstep became) and the threshold (`STILL_AIR`, `summitKind`), and `audio.js` imports it for `birdsSing`. Past it the silence stills the wind (0.132 down to 0.0044 at the bench under Xvfb) and lets go with a gust target of 1 in place of the crow, and the snap is `stoneFall`: five to seven knocks, pan -0.69 to 0.88 climbing and the mirror descending, matched against `mountainH` six metres either side. `smoke.mjs` 113 to 120, `test/browser.mjs` 96 to 103 (103/0 under Xvfb). Breaks from green, each caught by its own line. In smoke: birdsong to 0.7 (the agreement line, at y 55.10), the band's foot at 48, `STILL_AIR` 0.97 (green at first; the stretch floor went from 10% to 15% of the trail and then caught it at 11%), the stone's pan reversed, gaps widening, flat pitch and flat gain. In `browser.mjs`, in `git worktree` copies two at a time: the still leaving the wind at its normal gain plus the stone's ears swapped (those two lines alone); the release gust removed plus `summitKind` always true (the gust line and both low lines, and the wind line too, because the wind was already still when the bench read it); the silence never stilling plus the snap always a branch (the bench line, the wind, the gust and the stone). That last run died of a TypeError in the valley-ear line's detail string before the count printed; the line now says "no stone", and the re-run finished 103 checks, 5 failed, the valley-ear line among them.

## Signal City, R1, a reference hand and a no-input table, 2026-09-24
Roadmap R1, ¼, naming Opus 5, worked under Opus 5.5; ranked 13 and claimed on `main` in PR #398. `tools/calibrate.mjs` has `--baseline` (every level as it ships, no input) and `--hand` (holdPlatoon, the corridor on spawn, the longest-waited queue at each green's end, a corridor's offset swept at load; `--hand=phases,platoons,corridor,offset` plays only the parts named). Both print markdown tables, and both at once add a table of the hand against no input. The hand calls the World's own `requestPhase`, `holdGreen`, `requestPriority` and `setOffset`, and reads the controller's `timeToYellow` for a green's end. It copies no rule of the game (#34). The shape is #636 and the outcome #637. The baseline table reproduced the WISHLIST's evidence cell for cell: levels 1 to 3 lock 6/6, Crossing ★ 6/6, Two Blocks ★★★ 5/6, Rush Hour and Main Street ★★★ 6/6, School Run 2/6, Free Play 2/6. The hand: First Light ★★★ 5/6, Stem 4/6, Four Ways 2/6 (one lock), Crossing 1/6, Two Blocks 5/6 at offset 16, Rush Hour 5/6, School Run 1/6, Main Street 3/6 (one lock), Free Play 2/6 (two locks). The means, no input against hand: Crossing 59.5, 50.2 s against 66.5, 42.6 s; School Run 95.3, 11.2 s against 99.7, 13.2 s; the other four as #637. Two Blocks' sweep in stars over six seeds: 13, 15, 13, 15, **17**, 13, 15, 10, 14, 15, 8 at 0 to 40 s. The full run is about 25 minutes here: the sim is 5 to 17 s a run in this container, not the minute a sweep took before, and the sweep is 66 runs. It is a tool, so nothing new is in CI and no guard-rail was added. The nine suites are unchanged and green, 919 checks, as are `npm run check`, `social:check` and `ci-check.mjs`.

## Signal City, R2, the lesson decides a star, 2026-09-24 (PR #403)

Roadmap R2, ½, naming Fable 5.1, worked under Opus 5.5 (#638); ranked 13 and claimed on `main` in PR #402. The call is #639 and the old stars standing is #640. `scoring.js lessonMet` reads four kinds off new World stats (`carried`/`carriedStops` at the handoff, `ambulanceEscorted`/`ambulanceLateBy`, `pedMaxWait`); the level card names the second star and the end card has a lesson row. The audit of PR #399 found one bug, fixed in the first commit: a hold restarts the rule's clock, so `HAND_MAX` capped nothing on a level with a rule and greens ran about 44 s (Main Street seed 1's longest outside a platoon, 51.5 s to 22.0 s). The hand also learned to answer a walk call (`answerWalks`, only once the call has waited longer than every car, or Crossing's left bays starved into four locks), and to ask for a platoon's green 60 m or 10 s from the line, whichever is nearer. After the change, no input three-stars no starred level (Free Play, unstarred, 2/6). The hand: First Light 5/6, Stem 4/6, Four Ways 2/6 (one lock), Crossing 0/6, Two Blocks 4/6 at offset 16 (sweep 0 to 40 s: 6, 6, 6, 12, **15**, 13, 8, 11, 9, 7, 6 stars), Rush Hour 6/6, School Run 0/6 (one lock), Main Street 4/6, Free Play 3/6 (one lock). School Run is left open for R3 (#639). `test/stars.mjs` is new in Site CI, 12 checks in about two minutes on four cores. Breaks from green, each caught by its own line: Rush Hour's lesson removed (its star line alone, three stars on six seeds); a lucky green counted as the lesson (Rush Hour's star line on seeds 1, 4 and 6, its World line rightly green); Two Blocks back at 16 s (both Two Blocks lines). In `scoring.mjs`: `pedMaxWait` never kept, `carriedStops` never counted, platoons always met, and pending calls ignored (that last one only the "still waiting" line). In `browser.mjs`: the lesson row always "yes", the card line gone, the wait target kept. Suites: signals 176, sim 274, scoring 75 to 89, sprites 24, wave 23 to 24, campaign 35, grid 46, endless 39, stars 12, browser 227 to 233, 952 checks. `npm run check`, `social:check` and `ci-check.mjs` are green.

## Signal City, R3, hard mode on one level, 2026-09-24 (PR #405)

Roadmap R3, ¼, naming Fable 5.1, worked under Opus 5.5 (#638); ranked 13 and claimed on `main` in PR #404. The call is #641: Rush Hour ships `mode: 'hard'`, not School Run. `--baseline --hand`, collisions per seed: School Run no input 4, 5 and hand 4, 5, 6 with a lock on 3 (0/6 three stars); Main Street no input none and hand 4, 5 (4/6); Two Blocks no input 1 (twice), 3, 6 and hand at offset 16 on 1 (twice) (4/6); Rush Hour none either way, hand 6/6. The lesson played on the shipped rule (the corridor 2 s in) collides on Rush Hour seeds 2, 5 and 6 at 1.5 s of all-red, 0 of 6 at 2.5 s (seed 3 then clears 55 of 56) and 1 of 6 at 5 s. The audit of PR #403 found one thing, fixed in the first commit: #639 had grown to about ten sentences and is back to three. Its guard break (walks forced to met) failed School Run's star line in `test/stars.mjs` alone. New guards: `test/scoring.mjs` crashes a W and an S through on the shipped Rush Hour and Main Street through the World; Rush Hour's corridor run plays at 2.5 s and 1.5 s ends it; `test/browser.mjs` checks the card. Breaks from green: `mode: 'hard'` removed fails the "does not forgive one" line with the hard-level list and the 1.5 s line (all three read the mode); Rush Hour set soft fails the browser card line alone. Suites: signals 176, sim 274, scoring 89 to 95, sprites 24, wave 24, campaign 35, grid 46, endless 39, stars 12, browser 233 to 234, 959 checks. `npm run check`, `social:check` and `ci-check.mjs` are green.

## Signal City, R4, endless: measure the hand, then fix the ramp, 2026-09-24 (PR #409)

Roadmap R4, ½, naming Fable 5.1, worked under Opus 5.5 (#638); ranked 13 (60 after #408's re-rank) and claimed on `main` in PR #407. The call is #648, committed before the rule; it was written as #642 and renumbered when #408 took #642 to #647. `tools/calibrate.mjs --endless --hand` plays each city hands-off and then with `handStep` and prints both first missed days; `--m9` and `--events` replay the other ramps. First missed day, seeds 1 to 6, no input against the hand: M9's ramp 11, 12, 11, 11, 2, 10 against 11, 12, 10, 11, 6, 10; from day 3's district (ships) 10, 10, 6, 8, 2, 8 against 9, 10, 6, 8, 6, 9; the same with an event a day from day 4, 9, 7, 6, 8, 2, 8 against 9, 6, not run, 4, 4, 9. The hand's day-4 locks are box-blocking: an E through entered box 3 of seed 4 with its exit backed up from the next box, and a left behind it stalled 30 s. A hand that left out queues whose exit was full and cut a green over a car stalled 12 s in the box locked at the same 130.3 s; it was reverted, and the hand is R1's. The audit of PR #405 found one thing, fixed in the first commit: Rush Hour's hint sent a player to the corridor on the 1.5 s all-red that hard mode punishes, and now says to raise it first. Its guard break (Rush Hour soft) failed the "does not forgive one" line with the two others that read the mode. `test/endless.mjs` runs each city in a child process, first six days in Site CI (`--full` uncapped): N = 0 on five seeds of six, and seed 5 past the day no input misses. Breaks from green: events on fails the five-seed line alone (4 of 6; at the row's four it stayed green, which is why five); `World.requestPhase` refusing everything leaves both lines green, since a hold alone gets seed 5 through; with `holdGreen` refusing too, the second line alone fails. A first break in a scratch copy died of `ERR_MODULE_NOT_FOUND` (the copy lacked `assets/js/gvb-save.js`) and was rerun. `endless.mjs` is about eight minutes on three cores here, and Site CI's Signal City entry went from about 8 to 14 minutes, so it has `timeout: 30`. A best under M9's ramp keeps its days; `signal_city_v1` is unchanged. Suites: signals 176, sim 274, scoring 95, sprites 24, wave 24, campaign 35, grid 46, endless 39 to 42, stars 12, browser 234, 962 checks. `npm run check`, `social:check` and `ci-check.mjs` are green.

## Blender assets, the re-rank and the plans, 2026-09-25 (PR #408)

A planning session on Devon's instruction, docs only, worked under Opus 5.5; not a batch, so BACKLOG.md's last-batch line still names Signal City R3. Forty-seven Blender rows went in at ranks 1 to 47 and the thirteen before them moved to 48 to 60 in the same order (#642). The table grew a `Gate` column (#644), each project keeps its own `common.py` (#643), everything downstream of a render is cloud work (#645), a pack replaces only what a game shows (#646), and Torchbearer and Closing Time got no rows (#647). The shared plan is BACKLOG.md's "Blender assets: the common plan"; the rest is in each project's section or `WISHLIST.md`. What the survey corrected in the brief it started from: Golden Hour's animals are dolphin, gull, crab, heron, cormorant, owl, bat, pelican, sanderling and seal, and Blue Hour's are deer, fox, owl, crow, squirrel and small bird, with its bear a silhouette plane in `dread.js` and its elk a sound. School Generator's `gltf.js` reads vertex colour and no texture, skin or clip, which its pipeline has to write to. Aphelion's `test/desktop-input.mjs` fails on any second file in `libs/`, so its loader row widens that line by name. And 15 of the 25 `BoxGeometry` calls in Fourth Quarter's `world.js` are layout, not furniture. `npm run check`, `social:check` and `ci-check.mjs` are green.

## Signal City, R5, the site's side, 2026-09-25 (PR #413)

Roadmap R5, ¼, naming Sonnet 5, worked under Opus 5.5 (#638); rank 60, claimed on `main` in PR #412. `games.mjs` and `play-games.mjs` carry a `signal-city` entry: First Light from its card, a real `2` key, 20 s at 1x, the Signal line reads "Changed by you" and the clock ran 21 s, then the first car clears inside 65 s (#649; forty seeds put it 20.9 to 48.3 s in, so the row's cleared-at-20-s could not pass). Break: the key pointed at `requestPhase(n + 8)` fails the Signal line alone, reading "The level's opening phase". `capture-previews.mjs` steps Rush Hour on seed 7 to 95 s through `?debug` and asserts rush-hour, dusk and a 1.7 surge before it shoots; it also shoots a district of six, turned down at 330 px (#650). Promoted to 10.2 KB and 55.1 KB, and `npm run social` pointed the page's og:image at it. The card names the district and endless; the landing row shows the still. #585 folded in: `DUSK_LEVELS` is gone and Rush Hour carries `light: 'dusk'`; set to day, the browser suite fails "Rush Hour is played at dusk" alone. The audit of PR #409 found its suites at their stated counts and its five-seed line failing alone with events on; its log paragraph had no heading, added here. Suites: signals 176, sim 274, scoring 95, sprites 24, wave 24, campaign 35, grid 46, endless 42, stars 12, browser 234, 962 checks; `npm run games signal-city` 6. `npm run check`, `social:check` and `ci-check.mjs` are green.

## Golden Hour's GLTFLoader, 2026-09-25 (PR #416)

B2, ¼, naming Opus 5.5, worked under Opus 5.5 (#638); rank 2, claimed on `main` in PR #415. `libs/addons/loaders/GLTFLoader.js` and `libs/addons/utils/{BufferGeometryUtils,SkeletonUtils}.js` are three@0.185.0's `examples/jsm/` files byte for byte, from `npm pack` in a scratch folder, under #18's layout; the import map is unchanged. Two utils, not the plan's one (#651). `test/gltf-loader.mjs` opens the README as plain text, writes in a document carrying `index.html`'s import map verbatim, builds a GLB in the page and parses it: a TRIANGLES triangle comes back with 3 vertices, no index and bounds (0,0,0) to (2,3,1), and a TRIANGLE_STRIP quad comes back with 4 vertices and an index of 6, which is `toTrianglesDrawMode`'s work, so the util runs rather than only loads. 15 checks on port 8161; Site CI's Golden Hour entry gained `install: Tools/board-check`. Breaks from 15/15: deleting `BufferGeometryUtils.js` fails the import line ("Failed to fetch dynamically imported module: .../GLTFLoader.js") with the two mesh lines and the 404 behind it, 4 failed; deleting `SkeletonUtils.js` fails the same four; leaving the util in place with `toTrianglesDrawMode` returning its input fails the six-index line alone, reading `null`. Suites: smoke 135, gltf-loader 15. `npm run check`, `social:check` and `ci-check.mjs` are green.

## Blue Hour's GLTFLoader, 2026-09-25 (PR #419)

B2, ¼, naming Opus 5.5, worked under Opus 5.5 (#638); rank 5, claimed on `main` in PR #418. Confirmed first: `libs/three.core.js` reads REVISION '185' and index.html's import map maps only `three`, so the map is unchanged. `libs/addons/loaders/GLTFLoader.js` and `libs/addons/utils/{BufferGeometryUtils,SkeletonUtils}.js` are three@0.185.0's files byte for byte (identical to Golden Hour's copies, vendored separately under #17 and #18, both utils per #651). `test/gltf-loader.mjs` is Blue Hour's own copy of Golden Hour's, port 8162, PAGE Blue Hour's README; 15 checks. Site CI's Blue Hour entry gained `install: Tools/board-check` and the test; Tools/board-check/README.md's port table gained 8161 (Golden Hour's, missing since PR #416) and 8162. Breaks from 15/15: deleting BufferGeometryUtils.js fails the import line ("Failed to fetch dynamically imported module: .../GLTFLoader.js") with the two mesh lines and the 404 behind it, 4 failed, exit 1; `return geometry;` as the first line of toTrianglesDrawMode fails the six-index line alone, reading `null`, 14 passed, exit 1. Both restored and cmp'd against the pack. Suites: smoke 120, gltf-loader 15. `npm run check`, `social:check` and `ci-check.mjs` green. No new locked decision.

## The Fourth Quarter's GLTFLoader, 2026-09-25 (PR #422)

B2, ¼, naming Opus 5.5, worked under Opus 5.5 (#638); rank 12, claimed on `main` in PR #421. Confirmed first: `libs/three.module.js` reads REVISION '160', index.html's import map maps only `three`, and the r160 loader imports only `three` and `../utils/BufferGeometryUtils.js` (line 68), so no SkeletonUtils and the map is unchanged. `libs/addons/loaders/GLTFLoader.js` and `libs/addons/utils/BufferGeometryUtils.js` are three@0.160.0's files byte for byte (cmp against Tools/board-check/three-0.160.0/, #19; identical to Bell to Bell's committed blobs, whose Windows checkout differs only by CRLF, so the copies were taken from the pack). `test/gltf-loader.mjs` is the project's own copy of Blue Hour's (#17), port 8163, PAGE the project's README, expecting '160'; 15 checks. `.github/workflows/fourth-quarter-ci.yml` gained `install: Tools/board-check` so its `test/*.mjs` loop can import the harness (CI log shows gltf-loader 15 passed); Tools/board-check/README.md's port table gained 8163; WISHLIST.md's Blender ranks were relabelled from the stale 13 to 16 to the live ones. Breaks from 15/15: deleting BufferGeometryUtils.js fails the import line ("Failed to fetch dynamically imported module: .../GLTFLoader.js") with the two mesh lines and the 404 behind it, 3 passed 4 failed, exit 1; `return geometry;` as the first line of toTrianglesDrawMode fails the six-index line alone, reading `null`, 14 passed, exit 1. Both restored and cmp'd against the pack. Suites: gltf-loader 15, smoke-campaign 293, smoke-engine 223, smoke-events 121, smoke-layout 188, smoke-league 84, smoke-nav 129, smoke-regulars 98, smoke-textures 245. `npm run check`, `social:check` and `ci-check.mjs` green. No new locked decision.
