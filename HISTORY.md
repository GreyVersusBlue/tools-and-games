...
**#639. Signal City's second star is the lesson on a level with a default, not worse defaults, and Two Blocks ships on one clock.** `lesson: { kind, ... }` replaces the wait target: Rush Hour the ambulance on time with its corridor called, Main Street no split platoon, Two Blocks at most 50% of the cars one box hands on stopping again, School Run no walk call over 40 s. "On time" alone could not hold R2's rule, because no input's lucky green was on time on 3 of 6 and quicker than the hand's corridor (12.4 to 18.8 s against 12.6 to 36.5). The shipped 16 s offset already met any Two Blocks star, and wait could not separate offsets (0: 12.4 to 19.3 s; 16: 9.4 to 15.2), but the share stopping again could (0: 65 to 95%; 16: 19 to 71%). So it ships at 0, a slider untouched rather than a plan set wrong. Main Street's `boxStall` went from 45 to 60, because holding the procession, the lesson itself, kept a permissive left in the box 48 s. Levels 1 to 4 keep the wait star. The hand half of the rule is held on lesson levels only, since R1's hand never met it on Crossing or Four Ways. School Run misses it: its lesson is 0/6 with no input, but no walk move tried gets the hand past 2 of 6, and the one kept, which lets a call jump the queue only once it has waited longer than every car (it starved Crossing's left bays into four locks otherwise), three-stars it on 0 of 6 with one lock. Reversible per level by deleting `lesson`. *Signal City, R2.*
