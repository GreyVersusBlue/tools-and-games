...
The second increment of M9: endless. `js/endless.js` makes day n of a city `dayLevel(seed, n)`: the grid's n boxes (12 at most), demand 10% up a day, 180 s, a target of 20 plus 8 a day. The first target design followed each city's own demand and was thrown out on the calibration: past five boxes a district clears a flat 50 to 130 cars and at 21 days' traffic nothing locks, so only a climbing target ends a run (#608). The page has an Endless card after the levels, shut until Two Blocks has a star; a day's end card reads the day, the run and the best, and its button is Next day or Again. A new day is a new World with every old box's rules and timing carried (#610). The best goes in the save as `endless` through `repair` (#611), and `convertible` now refuses a grid (#612). `tools/calibrate.mjs --endless` prints the table. `test/endless.mjs` is new (39) and in Site CI; browser 194 to 209; the other seven suites unchanged. Every existing level and ring hashes the same as main on seeds 3 and 5. Decisions #607 to #612. Lesson: the first break of `fresh()` crashed the suite on a line that wrote to the missing field before any assertion looked at it; a crash is red but it is not the assertion that claims the break, so the fixture now comes through `repair`. And a copy check on carried rules stayed green against a break, because `setRules` copies anyway; it was deleted (#147).
