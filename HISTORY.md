...
**#642. Endless starts on the district's third day with no events, and its rule is N = 0: the reference hand lasts at least as long as no input on four seeds of six.** The hand's phase choice does not raise a district's capacity, which is where the target climbs to, so it outlasts no input only where the 20 s rule locks early (seed 5, by four days), and N = 1 fails four seeds of six on either ramp. Day 3's district cuts two idle days and moves nothing else; an event a day from day 4 is off because a surge box-blocked the hand on day 4 of seeds 4 and 5, so events punished playing. Reversible in `endless.js RAMP`; the tables are R4's log. *Signal City, R4.*
